...
# ---- 分段：每 300m 从井底向上 ----
TOP, BOT = 5469.7, 7746.0
STEP = 300.0
_bounds = [BOT]
while _bounds[-1] - STEP > TOP:
    _bounds.append(_bounds[-1] - STEP)
_bounds.append(TOP)
SEGS = [(_bounds[i + 1], _bounds[i], f"{_bounds[i + 1]:.0f} – {_bounds[i]:.0f} m")
...
def plot(segs, profile):
    ...
    for s in segs:
        c = GRADE_C[s["grade"]]
        top, bot, mid, val = s["top"], s["bot"], s["mid"], s["eff"]
        h = bot - top
    ...
    fig.suptitle("呼探1-001 井  尾管段固井顶替效率沿深度分布",
