...
输出：
- CSV汇总表: results/呼1-003_敏感性分析/呼1-003_敏感性分析结果.csv
- 单因素曲线图 ×3
- 两两交互热力图 ×3

使用方式：
    python scripts/probes/ht1_003_sensitivity.py
    python scripts/probes/ht1_003_sensitivity.py --resume  # 断点续跑
    python scripts/probes/ht1_003_sensitivity.py --plot-only  # 仅生成图表
"""
...
# 输出目录
OUTPUT_DIR = _PROJECT_ROOT / "results" / "呼1-003_敏感性分析"
CSV_PATH = OUTPUT_DIR / "呼1-003_敏感性分析结果.csv"
CHECKPOINT_PATH = OUTPUT_DIR / "呼1-003_敏感性分析_中间结果.csv"
    ...
    configs = [
        {
            ...
            "filename": "呼1-003_排量敏感性.png",
        },
        {
            "x_col": "屈服应力_Pa",
            "x_label": "尾浆屈服应力 (Pa)",
            "title": "屈服应力对全井有效顶替效率的影响",
            "filename": "呼1-003_屈服应力敏感性.png",
        },
        {
            "x_col": "黏度_mPas",
            "x_label": "尾浆塑性黏度 (mPa·s)",
            "title": "塑性黏度对全井有效顶替效率的影响",
            "filename": "呼1-003_黏度敏感性.png",
        },
    ]
...
def plot_heatmaps(df: pd.DataFrame) -> None:
    ...
    configs = [
        {
            ...
            "filename": "呼1-003_排量-屈服应力_热力图.png",
        },
        {
            "x_col": "排量_m3min",
            "y_col": "黏度_mPas",
        ...
        },
        {
            ...
            "filename": "呼1-003_屈服应力-黏度_热力图.png",
        },
    ]
    ...
    for cfg in configs:
        ...
        # 更新标题中的固定参数值为实际使用的值
        title_fixed_str = ", ".join(f"{k}={actual_fixed[k]:.0f}" for k in cfg["fixed"])
        ...
        ax.set_xticklabels([f"{v:.2f}" if v < 1 else f"{v:.0f}" for v in pivot.columns], fontsize=9)
...
# ============================================================
def main() -> None:
    parser = argparse.ArgumentParser(description="呼1-003井尾浆参数敏感性分析")
    parser.add_argument(
