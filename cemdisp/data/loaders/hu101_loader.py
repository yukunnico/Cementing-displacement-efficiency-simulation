...
MISSING = "MISSING"
# 读数（主检 2011122.pdf（W301-22094，89℃，θ600 未测）+ 复检 2011121.doc（93℃，θ600 未记录）｜300/200/100/6/3（ht1_003 含 600）：
HU101_LEAD_YIELD_STRESS_REPORT_GIVEN_PA = MISSING  # 路线A：报告未给定（只给 n/K）
HU101_LEAD_YIELD_STRESS_FITTED_PA = 0.9697  # 路线B 新推导：领浆 HB τy（读数 144/104/57/8/4，θ600 未测）
HU101_TAIL_YIELD_STRESS_REPORT_GIVEN_PA = MISSING  # 路线A：报告未给定（只给 n/K）
HU101_TAIL_YIELD_STRESS_FITTED_PA = 1.5111  # 路线B 新推导：尾浆 HB τy（读数 122/82/49/7/4，θ600 未测）
HU101_LEAD_RECHECK_YIELD_STRESS_REPORT_GIVEN_PA = MISSING  # 路线A：报告未给定（只给 n/K）
HU101_LEAD_RECHECK_YIELD_STRESS_FITTED_PA = 1.5182  # 路线B 新推导：领浆（复检） HB τy（读数 149/106/60/9/5，θ600 未记录）
HU101_TAIL_RECHECK_YIELD_STRESS_REPORT_GIVEN_PA = MISSING  # 路线A：报告未给定（只给 n/K）
HU101_TAIL_RECHECK_YIELD_STRESS_FITTED_PA = 1.2804  # 路线B 新推导：尾浆（复检） HB τy（读数 126/87/53/8/4，θ600 未记录）
...
#    archive/results_2026-09-14/_呼101_基线备份_居中度0.43_2026-09-11/
#    （2026-09-14 Task 2 results 归档时迁址，原 results/_呼101_基线备份_居中度0.43_2026-09-11/））。
    ...
    *,
    ...
    well_spec = WellSpec(
        ...
        notes=(
            ...
            "居中度 standoff 现行 LEGACY 0.38–0.48 名义剖面（model_assumption，均值 0.429）；"
            "2026-09-11 曾按用户指令临时改为全井常数 0.80（反推情景），已于 R30（2026-09-15）"
            "回退 LEGACY（三代留痕见本文件'居中度剖面三代记录'注释链）。⚠️ 现场记录为"
            "'悬挂器坐挂失败、座底固井，居中度下降'（132 只扶正器布置，11–22m 间距，无实测居中度）；"
            "0.80 常数口径的任何数字不得作为模型验证数字引用。",
