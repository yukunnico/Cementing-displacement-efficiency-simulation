...
MISSING = "MISSING"
# 读数（HT1-004 油层尾管化验报告 表7，132℃↓93℃（θ600 记 `>300` 超量程，已剔除）｜300/200/100/6/3（ht1_003 含 600）：
HT1_004_LEAD_YIELD_STRESS_REPORT_GIVEN_PA = MISSING  # 路线A：报告未给定（只给 n/K）
HT1_004_LEAD_YIELD_STRESS_FITTED_PA = 0.0000  # 路线B 新推导：领浆 HB τy（读数 296/213/120/10/6，θ600 超量程）
HT1_004_TAIL_YIELD_STRESS_REPORT_GIVEN_PA = MISSING  # 路线A：报告未给定（只给 n/K）
HT1_004_TAIL_YIELD_STRESS_FITTED_PA = 0.0000  # 路线B 新推导：尾浆 HB τy（读数 293/211/120/10/5，θ600 超量程）
# 注意：本井**生产口径的水泥相是 BINGHAM**（优化参数化 PV0.17/YP13.0、PV0.18/YP14.0，
# 见上方 HT1_004_LEAD_PV_PA_S/YP_PA），与本行 τ_y 不同源，两者不得互相替代；
# 本行只供 Task 6 的 `hb_fix_cement_tau_y`（默认 False）消费。
