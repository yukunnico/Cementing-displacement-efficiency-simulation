    ...
    *,
    ...
    well_spec = WellSpec(
        ...
        notes=(
            ...
            "管容链现场核实（2026-09-02）：shoe_lag_volume_m3=88.55m³ 为全井管容实测链（0–鞋口：149.2 送入钻杆 "
            "ID129.9 30.98 + 127 钻杆 ID107.7 28.7 + 悬挂器过渡 ~0.15 + 尾管 168.3 ID138.9 27.03 + "
            "139.7 ID107.94 3.49+球座下 0.53），20315 灌水试验+流量计双证；20313.txt 行490 设计表合计 "
            "90.2（含后置液）/90.7 m³ 并存，实测口径优先；实际替浆 90.9 m³ 到量碰压成功（20314.doc）。",
        ),
    )
