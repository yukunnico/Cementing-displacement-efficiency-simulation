...
# 呼103 全井管容链（2026-09-02 现场核实修复）：shoe_lag_volume_m3 直接驱动
# casing_flow._timeline_pipe_volume 与 _pipe_cross_section_area 双链，统一到现场真值。
# 实测链组成（20313.txt 行490 管容表 + 20315 灌水/流量计双证）：
#   149.2 送入钻杆 ID129.9（0-2392m，30.98m³）+ 127 钻杆 ID107.7（2392-5536.6m，28.7m³）
#   + 悬挂器过渡（~0.15m³）+ 尾管 168.3 ID138.9（27.03m³）+ 139.7 ID107.94（3.49+球座下0.53m³）。
HU103_SHOE_LAG_VOLUME_M3 = 88.55
# 双口径并存：20313.txt 行490 设计表合计 90.2（含后置液）/90.7 m³；实测 88.55 优先
# （20315 灌水试验+流量计双证）；实际替浆 90.9 m³ 到量碰压成功（20314.doc）。
...
MISSING = "MISSING"
# 读数（203111.docx 表7（θ600 原文记 `/` 未记录）｜300/200/100/6/3（ht1_003 含 600）：
HU103_LEAD_YIELD_STRESS_REPORT_GIVEN_PA = MISSING  # 路线A：报告未给定（只给 n/K）
HU103_LEAD_YIELD_STRESS_FITTED_PA = 0.0000  # 路线B 新推导：领浆 HB τy（读数 224/158/93/9/5，θ600 未记录）
HU103_INTERMEDIATE_YIELD_STRESS_REPORT_GIVEN_PA = MISSING  # 路线A：报告未给定（只给 n/K）
HU103_INTERMEDIATE_YIELD_STRESS_FITTED_PA = 0.4961  # 路线B 新推导：中间浆 HB τy（读数 254/179/112/14/7，θ600 未记录）
HU103_TAIL_YIELD_STRESS_REPORT_GIVEN_PA = MISSING  # 路线A：报告未给定（只给 n/K）
HU103_TAIL_YIELD_STRESS_FITTED_PA = 0.8967  # 路线B 新推导：尾浆 HB τy（读数 265/186/115/15/7，θ600 未记录）
...
def load_hu103_tailpipe(
    *,
    ...
    well_spec = WellSpec(
        ...
        # 全井管容链现场核实值（2026-09-02）：实测 88.55m³（20315 灌水+流量计双证），
        # 驱动 casing_flow._timeline_pipe_volume 与 _pipe_cross_section_area 双链。
        shoe_lag_volume_m3=HU103_SHOE_LAG_VOLUME_M3,
        notes=(
            ...
            "管容链现场核实（2026-09-02）：shoe_lag_volume_m3=88.55m³ 为全井管容实测值（20315 灌水试验+"
            "流量计双证核实）。分段理论链（0–鞋口）：149.2 送入钻杆 ID129.9 30.98 + 127 钻杆 ID107.7 28.7 + "
            "悬挂器过渡 ~0.15 + 尾管 168.3 ID138.9 27.03 + 139.7 ID107.94 3.49+球座下 0.53 ≈ 90.88m³，"
            "与 20313.txt 行490 设计表 90.2（含后置液）/90.7 m³ 并存；实测 88.55 与理论差 ≈2.3m³，"
            "按实测口径取值；实际替浆 90.9 m³ 到量碰压成功（20314.doc/20315）。",
        ),
    )
