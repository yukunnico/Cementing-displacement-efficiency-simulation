...
WELL_FLUID_PROVENANCE: dict[str, dict[str, FluidProvenance]] = {
    ...
    "呼102": {
        ...
        "平衡液": FluidProvenance("field", "密度1.90g/cm³现场实测（20211.doc）；流变无实测，PV/YP 沿用邻井代理。"),
        "隔离液": FluidProvenance("field", "密度2.05g/cm³现场实测（20211.doc）；流变无实测，PV/YP 沿用邻井代理。"),
        ...
        "后置液": FluidProvenance("partial", "密度1.90g/cm³现场实测（20211.doc）；流变无实测，复用钻井液（proxy）。"),
    ...
    "呼103": {
        ...
        "隔离液1": FluidProvenance("partial", "设计密度2.00g/cm³（20313.doc 7.1.2）；流变无实测，沿用代理。"),
        "隔离液2": FluidProvenance("partial", "设计密度1.95g/cm³（20313.doc 7.1.2）；流变无实测，沿用代理。"),
        "隔离液(实际)": FluidProvenance("field", "实际密度1.92g/cm³（20314.doc）；流变无实测，沿用代理。"),
        "领浆": FluidProvenance("field", "现场密度2.05g/cm³与幂律参数n=0.82,K=0.67，来自203111.docx实验报告（A级）。"),
        "中间浆": FluidProvenance("field", "密度2.05g/cm³、n=0.76、K=1.11来自实验报告（203111.docx）；体积按设计36m³（20313.doc 5.2.4）。"),
    ...
    "呼探1": {
        "钻井液": FluidProvenance("field", "密度2.12g/cm³固井前油基钻井液，PV=66mPa·s、YP=10Pa 现场实测（204111.doc）。"),
        "冲洗液": FluidProvenance("partial", "密度2.00g/cm³现场（204151.doc 冲洗型隔离液15m³）；流变无实测，沿用同区块隔离液代理（proxy）。"),
        "加重隔离液": FluidProvenance("partial", "密度2.05g/cm³现场（204151.doc 加重隔离液15m³）；流变无实测，沿用同区块隔离液代理（proxy）。"),
        "领浆": FluidProvenance("partial", "密度2.10g/cm³现场（204151.doc/204111.doc）；幂律 n/K 无呼探1实测，沿用同区块同体系实验值 proxy（呼102 领浆 n0.737/K0.947）。"),
        "尾浆": FluidProvenance("partial", "密度1.90g/cm³现场（204151.doc/204111.doc）；幂律 n/K 无呼探1实测，沿用同密度体系实验值 proxy（呼1-001 尾浆 n0.886/K0.453）。"),
        "后置液": FluidProvenance("partial", "密度2.05g/cm³现场（204151.doc 后置液2m³）；流变无实测，复用隔离液代理（proxy）。"),
        "替浆液1": FluidProvenance("partial", "密度1.95g/cm³现场（204151.doc 管串36m³）；流变无实测，复用钻井液实测（proxy）。"),
        "替浆液2": FluidProvenance("partial", "密度2.08g/cm³现场（204151.doc 环空49m³）；流变无实测，复用钻井液实测（proxy）。"),
    },
    "呼探1-002": {
