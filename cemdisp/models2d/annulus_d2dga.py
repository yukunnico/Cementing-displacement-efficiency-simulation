...
本模块实现偏心环空二维顶替效率计算的D2DGA（双液双区面积）模型。
...
class AnnulusD2DGASolver:
    ...
    实现偏心环空中水泥浆顶替钻井液的两相流动模拟。

    主要物理过程：
    1. 平流输运：水泥浆随速度场向下游运移
    2. 弥散效应：横向(day)和垂向(dax)扩散，与偏心度和流体性质相关
    3. 壁面清除：泥饼在剪切作用下被水泥浆逐渐清除
    ...
    def _pick_fluids(self, fluids: Tuple[FluidSpec, ...]) -> Tuple[FluidSpec, FluidSpec]:
        """从流体列表中选取钻井液和水泥浆。"""
        mud = next((fluid for fluid in fluids if fluid.role == fluid.role.MUD), None)
        cement = next((fluid for fluid in fluids if fluid.role in {fluid.role.LEAD, fluid.role.TAIL}), None)
        if mud is None or cement is None:
            raise ValueError("首版求解器至少需要钻井液和一个水泥浆流体")
        return mud, cement
    ...
    def _compute_props(self, cement: Array, w_prev: Array, geom: Dict[str, Array], mud_fluid: FluidSpec, cement_fluid: FluidSpec) -> Tuple[Array, Array, Array]:
        """计算混合物系的表观粘度、密度和钻井液分数。"""
        mud = np.clip(1.0 - cement, 0.0, 1.0)
        gamma = np.maximum(6.0 * np.abs(w_prev) / np.maximum(geom["b"], 1.0e-5), 1.0e-6)
        mu = mud * self._apparent_viscosity(mud_fluid, gamma) + cement * self._apparent_viscosity(cement_fluid, gamma)
        rho = mud * (mud_fluid.density_kg_m3 / 1000.0) + cement * (cement_fluid.density_kg_m3 / 1000.0)
        return mu, rho, mud

    def _compute_velocity(self, cement: Array, geom: Dict[str, Array], q_m3s: float, w_prev: Array, mud_fluid: FluidSpec, cement_fluid: FluidSpec) -> Tuple[Array, Array, Array, Array, Array]:
        """计算环空速度场。
        ...
        b = geom["b"]
        q_half = q_m3s / 2.0
        mu, rho, mud = self._compute_props(cement, w_prev, geom, mud_fluid, cement_fluid)
    ...
    def _depth_profiles(self, geom: Dict[str, Array], cement: Array, wall: Array) -> pd.DataFrame:
        """计算深度方向的平均剖面数据。"""
        eff = cement * (1.0 - wall)
        return pd.DataFrame(
            {
                ...
                "钻井液平均浓度": 1.0 - np.average(cement, axis=0, weights=geom["b"]),
                "宽边有效效率": eff[0],
    ...
    def run(
        ...
        mud_fluid, cement_fluid = self._pick_fluids(fluids)
        ...
        for step_index in range(int(self.total_t / self.dt) + 1):
            ...
            if pump_active:
                ...
                w, v, mu, rho, mud = self._compute_velocity(
                    cement, geom, inlet_state.flow_rate_m3_s, w_prev, mud_fluid, cement_fluid
                )
                w_prev = w
                ...
                w, v, mu, rho, mud = self._compute_velocity(
                    cement, geom, 0.0, w_prev, mud_fluid, cement_fluid
                )
        ...
        depth_profiles = self._depth_profiles(geom, cement, wall)
        ...
        return AnnulusSimulationResult(
            ...
            notes=(
                "首版仅使用单水泥浆相（尾浆）与钻井液两相。",
                "前置液/隔离液/领浆暂未纳入首版环空求解。",
            ),
        )
