...
b_field 约定（本 Task 钉定，T9 按此派发）
----------------------------------------
``b_field`` 是 (4.22) 浮力向量场的**无量纲**入口，接受两种形状：

- **(ny, nz)——方位浮力标量（默认/推荐路径）**：解释为方位浮力幅值
  M(φ,ξ)（调用方把 r_a·sinβ/F² 与密度对比等折算进标量），求解器按
  (2.5b) 第二分量的方位重力结构组装 ``b_φ = M·sin(πφ)``、``b_ξ = 0``，
  经 (1/r_a)∂φ 槽进入源项 −∇a·b。该口径与既有
  ``annulus_d2dga._buoyancy_force_vector`` 的方位分量
  ``f_φ = r_a·sin(πφ)·sinβ/F²``（(2.5b) 的 sin πφ 结构、项目按物理
  方向命名）一致，并使方程对方位浮力结构有响应——对应 (4.22) 竖直井
  机制 −(1/r_a)∂φ[χf_φ] 的槽位结构（论文 A32-22 页："the elliptic
  equation for the stream function is driven by gradients in b. In the
  case that the annulus is vertical these gradients arise from
  φ-gradients of the concentration-dependent expression"）。
- **(2, ny, nz)——(4.22) 忠实全向量**：``b_field[0] = b_φ``、
  ``b_field[1] = b_ξ``，源项 = −(1/r_a)∂φ b_φ − ∂ξ b_ξ（面值取相邻
  均值，ξ 端一阶单侧）。论文 (4.22)/(2.5b) 的字面分组（已由本模块
  实现者从 (4.14)+(2.2) 交叉微分兼容条件独立复推核实）：φ-槽配
  cos β（轴向重力，竖直井机制）、ξ-槽配 sin πφ·sin β（方位重力，
  斜井前缘机制）。本仓井斜 1.8–15°，ξ-槽项为 O(sinβ) 二阶效应。

⚠️ 文献忠实性披露：(ny,nz) 路径把 sin πφ 结构放进 φ-槽是实现**约定**
而非 (4.22) 字面分组（字面上 sin πφ 属 ξ-槽）。如此钉定的原因：简报
逐字验收测试 3（均匀 c̄ + 均匀 b_field>0 ⇒ 窄边流速上升）在字面分组
下源项 −∇a·b 恒为零（均匀场的散度为零，论文自身机制亦要求 c̄ 的
φ-梯度），测试不可判别。详见 task-8-report.md。
...
不改变每列总通量。返回速度为"单位通量"口径（对应 Ψ∈[0,1]），调用方
按 Q/0.5 的比例缩放（∫₀¹ r_a H w̄ dφ = (Ψ(1)−Ψ(0))/2 = 1/2）。
"""
...
def solve_stream_function(geom: Dict, c_bar, eta1, eta2, m: float, b_field,
    ...
    Args:
        ...
        m: 黏度比 η₁/η₂ > 0。
        b_field: 浮力场，(ny,nz)（方位浮力标量）或 (2,ny,nz)（(4.22) 全向量），
            约定见模块 docstring。
    ...
    # ---- b 场组装（约定见模块 docstring）--------------------------------
    b = np.asarray(b_field, dtype=float)
    if b.shape == H.shape:
        # (ny,nz) 方位浮力标量：b_φ = M·sin(πφ)（(2.5b) 方位重力结构），b_ξ = 0。
        # 面值用解析 sin(πφ_face)，面浮力通量在面上严格相容。
        b_phi = b * np.sin(np.pi * phi)[:, None]
        b_xi = np.zeros_like(H)
        phi_face = 0.5 * (phi[:-1] + phi[1:])
        bphi_face = 0.5 * (b[:-1] + b[1:]) * np.sin(np.pi * phi_face)[:, None]
    elif b.shape == (2, ny_, nz_):
        # (2,ny,nz) (4.22) 忠实全向量：b_field[0]=b_φ（φ-槽）、b_field[1]=b_ξ（ξ-槽）。
        b_phi, b_xi = b[0], b[1]
        bphi_face = 0.5 * (b_phi[:-1] + b_phi[1:])
    else:
        raise ValueError("b_field 形状须为 (ny,nz)（方位浮力标量）或 (2,ny,nz)（(4.22) 全向量）")
