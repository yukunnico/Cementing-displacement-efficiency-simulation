...
def plot_reference_depth_profile(
    ...
    ax.plot(profiles["水泥平均浓度"], profiles["井深_m"], label="周向平均", linewidth=2.3)
    ax.plot(profiles["宽边水泥浓度"], profiles["井深_m"], label="宽边", linewidth=2.0)
    ax.plot(profiles["窄边水泥浓度"], profiles["井深_m"], label="窄边", linewidth=2.0)
    ax.set_title("最终水泥体积分数深度剖面", fontsize=14, fontweight="bold")
    ax.set_xlabel("水泥体积分数")
    ax.set_ylabel("井深 / m")
...
def plot_reference_front_progress(
    ...
    fig, ax = plt.subplots(figsize=(9, 5.4))
    ax.plot(metrics["time_min"], wide_md, label="宽边前沿", linewidth=2.2)
    ax.plot(metrics["time_min"], mid_md, label="中线前沿", linewidth=2.0)
    ax.plot(metrics["time_min"], narrow_md, label="窄边前沿", linewidth=2.2)
...
def plot_reference_segment_efficiency(
    ...
    bars = ax.bar(labels, values, color="#2f80ed", edgecolor="#333333", linewidth=0.6)
    ax.set_title("分段最终顶替效率", fontsize=14, fontweight="bold")
    ax.set_ylabel("最终有效顶替效率")
    ax.set_ylim(0.0, 1.05)
