...
严格按呼103现场主作业运行 1D-2D 耦合模式并导出中文命名结果：
地面开泵 → 套管内前沿追踪 → 鞋口出流 → 环空入口 → 环空二维顶替

输出目录：results/呼103尾管_1D2D耦合模型/
...
def run_and_export(
    *,
    ...
    # 导出CSV
    metrics_path = output_dir / f"呼103尾管_{mode_title}_时间序列结果.csv"
    profiles_path = output_dir / f"呼103尾管_{mode_title}_深度剖面.csv"
...
def _export_casing_flow_timing(
    *,
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    timing_path = output_dir / "呼103尾管_1D2D耦合模型_鞋口出流时序.csv"
...
def run_hu103_tailpipe_initial() -> None:
    """呼103尾管段现场实录 1D-2D 耦合模型运行入口。"""

    well_spec, fluids, schedule, _ = load_hu103_tailpipe()
    output_dir = PROJECT_ROOT / "results" / "呼103尾管_1D2D耦合模型"
