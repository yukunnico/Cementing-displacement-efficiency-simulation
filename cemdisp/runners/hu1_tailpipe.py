...
def run_and_export(
    *,
    ...
    参数：
        ...
        output_dir: 结果输出目录，默认入口使用 results/呼探1尾管_1D2D耦合模型/。
        inlet_provider: 由 1D 鞋口出流桥接得到的环空入口边界状态提供器。
        total_t_s: 环空二维顶替计算总时长（秒），由替浆液到达鞋口时刻确定。
    """

    output_dir.mkdir(parents=True, exist_ok=True)
    well_spec, fluids, _, _ = load_hu1_tailpipe()

    # 呼探1严格现场模式下，二维求解时长由 1D 套管内前沿追踪确定：
    ...
    metrics_path = output_dir / f"呼探1尾管_{mode_title}_时间序列结果.csv"
    profiles_path = output_dir / f"呼探1尾管_{mode_title}_深度剖面.csv"
    ...
    summary_json_path = output_dir / f"呼探1尾管_{mode_title}_结果摘要.json"
    ...
    # 导出 2D 场数据 NPZ。呼探1为多浆柱体系，必须包含 lead/tail 快照和最终场。
    npz_path = output_dir / f"呼探1尾管_{mode_title}_2D场数据.npz"
    _ = np.savez(
        npz_path,
...
def _export_casing_flow_timing(
    *,
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    timing_path = output_dir / "呼探1尾管_1D2D耦合模型_鞋口出流时序.csv"
