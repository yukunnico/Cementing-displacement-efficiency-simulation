"""
呼1-004井（HT1-004）尾管段顶替效率模型运行器

模拟域：悬挂器(5243m) → 鞋底(7660m)，含套管重叠段+裸眼段。
严格按呼1-004井（HT1-004）现场主作业运行 1D-2D 耦合模式并导出中文命名结果：
...
def run_and_export(
    *,
    ...
    参数：
        mode_title: 模式标题（如"1D2D耦合模型"），用于文件名和打印
        output_dir: 结果输出目录
    ...
    # 导出CSV
    metrics_path = output_dir / f"呼1-004_{mode_title}_时间序列结果.csv"
    profiles_path = output_dir / f"呼1-004_{mode_title}_深度剖面.csv"
    ...
    fluid_provenance_summary = build_injected_fluid_provenance_summary(
        well_spec.well_name, schedule, fluids
    )
    summary_payload = dict(result.summary)
    summary_payload["注入流体现场符合性检查"] = fluid_provenance_summary

    # 导出JSON摘要
    summary_json_path = output_dir / f"呼1-004_{mode_title}_结果摘要.json"
    _ = summary_json_path.write_text(
        json.dumps(summary_payload, ensure_ascii=False, indent=2), encoding="utf-8"
    )

    # 导出Markdown摘要
    summary_md_path = output_dir / f"呼1-004_{mode_title}_结果摘要.md"
    final_result = cast(Mapping[str, float], result.summary["最终结果"])
    _ = summary_md_path.write_text(
        "\n".join(
            [
                f"# 呼1-004 {mode_title}结果摘要",
                "",
...
def _export_casing_flow_timing(
    *,
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    timing_path = output_dir / "呼1-004_1D2D耦合模型_鞋口出流时序.csv"
    ...
    annulus_start_time_s = (
        cement_arrival_time_s if cement_arrival_time_s is not None else pump_end_time_s
    )
    ...
    with timing_path.open("w", encoding="utf-8-sig", newline="") as handle:
        ...
        for time_s in sample_times:
            ...
            inlet_fluid = (
                state.phase_fractions[0][0] if state.phase_fractions else "未知"
            )
            ...
            writer.writerow(
                {
                    ...
                    "鞋口出流流体": inlet_fluid,
                    "环空入口相": "水泥相" if inlet_fluid == "尾浆" else "泥浆相",
                    "排量_m3_min": f"{state.flow_rate_m3_s * 60.0:.6f}",
                }
            )


def run_ht1_004_tailpipe_initial() -> None:
    """呼1-004井（HT1-004）尾管段现场实录 1D-2D 耦合模型运行入口。"""
