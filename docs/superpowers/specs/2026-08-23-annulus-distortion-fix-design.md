...
- **`_compute_velocity` 返回值扩元（固定 12 元组）**：现返回 8 元组（:673），固定扩为 `w, v, mu_reg, rho, mud, Re, mu_turbulent, m_field, tau_y, eta2, n_mix, kappa_mix`。**返回长度固定、不用条件长度**（否则解包会炸）；开关全关时新增 4 场始终计算（加权运算极轻）但不参与 pref。同步改 3 处解包：`:805`（泵注）、`:933`（停泵）、`tests/test_yield_deadzone.py:209`。`_compute_props` 由 7 元组扩为 9 元组（加 n_mix/kappa_mix）。tau_y 供 M3、eta2 供 I3、n_mix/kappa_mix 供 M2 复用，避免重复计算。
...
**重跑节奏（Q3-c，2026-08-23 裁定）**：M0 一轮（零物理，验证即过）→ M1 一轮（独立归因）→ M3+M2 合并一轮 → M4 收尾一轮；全量 8 井。开发期全程 hu101 三剖面快检。**M4 必须在 M3 就位后再开 e=0.90**（防 529:1 重演）。
...
| `:673` | `_compute_velocity` 返回 8 元组（固定扩为 12 元组，供 M2/M3/I3） |
