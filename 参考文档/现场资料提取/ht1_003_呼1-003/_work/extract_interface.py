# -*- coding: utf-8 -*-
"""HT1-003 CBL 矢量数字化：一界面/二界面解释填充 → 深度区间 + 质量分级 + 逐米class。"""
...
def main():
    JOBS = [
        ("HT1-003_固井质量测井评价图_3500-7595_20260622_完井.pdf", "main", 3500, 7595),
        ("HT1-003_固井质量测井评价图_15-3500_20260622_完井.pdf", "shallow", 15, 3500),
    ]
    all_rows = []
    for fn, tag, dmin, dmax in JOBS:
        ...
        page = doc[0]
        k, b = get_calib(page)
        print(f"=== {tag} ({dmin}-{dmax}m) calib k={k:.5f} b={b:.2f}  ({1/k:.2f} pt/m)")
        for name, x0, x1 in [("一界面", 688.5, 719.7), ("二界面", 719.7, 750.9)]:
            ...
            if tot > 0:
                print(f"    [{nseg} 段] 合计={tot:.1f}m  中等及以上占比={good/tot*100:.1f}%")
            else:
                print(f"    无填充（全空套管/未评价）")
        doc.close()
